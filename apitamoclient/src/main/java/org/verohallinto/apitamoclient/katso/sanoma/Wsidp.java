...
/**
 * <p>
 * AuthnRequest-sanoman l�hetys ja vastaussanoman k�sittely.
 * </p>
...
public class Wsidp {
  ...
  /**
   * AuthnRespond-sanoman XPath-k�sittelyss� k�ytett�v�t Namespace-m��ritykset
   */
  ...
  /**
   * SASLRequest-sanoman l�hetys.
   * </p>
   *
   * @param katso
   *          {@code KatsoDto} k�ytt�j�n tiedot.
   * @return Vastaus sanoma AuthnRequest-pyynt��n.
   */
    ...
    try {
      ...
      if (Props.lokitaSanomat()) {
        LokiApu.lokitaSanoma(
            LokiApu.annaEtuliite(Vakiot.KATSO_PYYNTO_WSIDP) + "Pyynto_" + System.currentTimeMillis() + ".xml", pyynto);
      }
      ...
      if (Props.lokitaSanomat()) {
        LokiApu.lokitaSanoma(
            LokiApu.annaEtuliite(Vakiot.KATSO_PYYNTO_WSIDP) + "Vastaus_" + System.currentTimeMillis() + ".xml",
            vastaus);
      }
   ...
   * @param katso
   *          {@code KatsoDto} k�ytt�j�n tiedot.
   * @return AuthnRequest-sanoma k�ytt�j�n tiedoilla.
   * @throws RuntimeException
   *           AuthnRequest-sanoman allekirjoitus ep�onnistui.
   */
  private static SOAPMessage muodostaPyynto(final KatsoDto katso, final XPath xp)
    ...
    // Poistetaan alkuper�inen header
   ...
   *
   * @param katso
   *          {@code KatsoDto} Katso-tunnistautumisessa k�ytett�v�t tiedot..
   * @param vastaus
   *          {@code SOAPMessage} vastaanotettu sanoma.
   * @param xp
   *          {@code XPath} XPath-instanssi.
   * @throws javax.xml.xpath.XPathExpressionException
   *           Sanoman XPath-k�sittelyss� tapahtui virhe.
   * @throws javax.xml.soap.SOAPException
   *           Sanoman SOAPBody-elementin haussa tapahtui virhe.
   * @throws java.text.ParseException
   *           P�iv�yksen parsinta ep�onnistui.
   * @throws RuntimeException
   *           Vastaussanoman k�sittelyss� tapahtui virhe.
   */
  private static void kasitteleVastaus(final KatsoDto katso, final SOAPMessage vastaus, final XPath xp)
    ...
    // Tarkastetaan l�hetetyn ja vastaanotetun sanoman id:t
    ...
    if (responseTo != null && responseTo.equals(katso.getAuthnMsgID())) {
      ...
      // txt.205=AuthnRespond: Tunnistautuminen ep�onnistui. Virheellinen {0}:
      // Odotettu='{1}', palautettu='{2}'.
   ...
   * @param doc
   *          {@code Document} vastaanotetun sanoman SOAPBody-elementti
   *          XML-dokumenttin�.
   * @param xp
   *          {@code XPath} XPath-instanssi.
   * @throws javax.xml.xpath.XPathExpressionException
   *           Sanoman XPath-k�sittelyss� tapahtui virhe.
   * @throws RuntimeException
   *           Tunnistautuminen ep�onnistui tai Sanoman
   *           'samlp:StatusCode'-elementin 'Value'-attribuutti puuttuu tai se
   *           on tyhj�.
   */
  private static void samlpStatusCodeOk(final String kieli, final Document doc, final XPath xp)
    ...
    if (status != null) {
      if (!Vakiot.KATSO_SUCCESS.equalsIgnoreCase(status)) {
        // txt.206=AuthnRespond: Tunnistautuminen ep�onnistui. Elermentti
        // 'samlp:StatusCode'='{0}'.
        throw new RuntimeException(Props.teksti(kieli, 206, new String[] { status }));
      }
    } else {
      // txt.204=AuthnRespond-sanoman '{0}'-elementti tai -attribuutti puuttuu
      // tai se on tyhj�.
  ...
  /**
   * AuthnRespond-sanoman 'saml:Assertion'-elementin tarkasta. Mahdollinen virhe
   * palautetaan SoapVastausDto-objektissa.
   * </p>
   *
   * @param katso
   *          {@code KatsoDto} Katso-tunnistautumisessa k�ytett�v�t tiedot..
   ...
   *           Sanoman SOAPBody-elementin haussa tapahtui virhe.
   * @throws RuntimeException
   *           Sanoman 'saml:Assertion'-elementti puuttuu tai se on tyhj�.
   */
  ...
  /**
   * AuthnRespond-sanoman 'saml:Attribute'-elementtien k�sittely.
   * </p>
   * <ul>
   * K�sitelt�v�t attribuutit:
   * <li>tfi.kid</li>
   ...
   *          {@code XPath} XPath-instanssi.
   * @param n
   *          {@code Node} 'saml:Assertion'-elementti.
   * @throws javax.xml.xpath.XPathExpressionException
   *           Sanoman XPath-k�sittelyss� tapahtui virhe.
   * @throws RuntimeException
   *           Sanoman 'saml:Attribute'-elementin tietoja ei l�ydy tai tiedot
   *           ovat puutteelliset.
   */
  private static void samlAttribute(final KatsoDto katso, final XPath xp, final org.w3c.dom.Node n)
    ...
    if (tfiKid != null && tfiPersonname != null) {
      ...
      // txt.208=AuthnRespond-sanoman '{0}' tai '{1}' -elementtien tietoja ei
      // l�ydy tai tiedot ovat puutteelliset.
      throw new RuntimeException(
          Props.teksti(katso.getIn().getKieli(), 208, new String[] { XPATH_TFI_KID, XPATH_TFI_PERSONNAME }));
    }
