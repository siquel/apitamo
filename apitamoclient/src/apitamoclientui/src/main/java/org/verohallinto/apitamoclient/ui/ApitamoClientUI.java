...
/**
 * <p>Esimerkkitoteutus ApitamoClient -luokan k‰ytˆst‰</p>
 * (c) 2014 Tietokarhu Oy
 * Date: 25.6.2014 - 13:34
 * $Rev$
 */
public class ApitamoClientUI {
   		    	...
   		    	if (cmd != null && cmd.length() > 0) {
   			        if (cmd.equalsIgnoreCase("login")) {
                           ...
                           if (!katsoOut.onTunnistusOk()) {
                               ...
                               //k‰ytt‰j‰ on tunnistautunut mutta tutkitaan ett‰ assertio on voimassa
                           ...
                           if (katsoOut.onTunnistusOk()) {
                               ...
                               if (apitamoOut != null) {
                                   if (apitamoOut.isLahetysTapahtumaOk()) {
                                       ...
                                       System.out.println("N‰ist‰ oikeellisia oli: " + apitamoOut.getTamoTulos().getOikeellisia() + "kpl.");
                                       System.out.println("N‰ist‰ virheellisi‰ oli: " + apitamoOut.getTamoTulos().getVirheellisia() + "kpl.");
                                       System.out.println("");

                                       if (apitamoOut.isAineistonVastaanottoOk()) {
                                           System.out.println("L‰hetetyt ilmoitukset on siirretty Verohallinnon k‰sittelyyn.");
                                           System.out.println("Tarkemman erittelyn n‰et tiedostosta: ." + apitamoOut.getVastausSanomanNimi());
                                           System.out.println("");
                                       } else {
                                           System.out.println("Aineistoa ei vastaanotettu. Syy: " + apitamoOut.getInfo());

                                           if (props.puraTamoTulos() &&
                                                   (apitamoOut.getTamoTulos().getLomakkeet() != null && apitamoOut.getTamoTulos().getLomakkeet().size() > 0)) {

                                               for (LomakeDto lomake : apitamoOut.getTamoTulos().getLomakkeet()) {
                                                   String tila = lomake.isLomakkeenTilaOk() ? "Ok" : "Virhe";
                                                   System.out.println("Asiakas: " + lomake.getyTunnus() + ", Lomake: " + lomake.getSelite() + ", Tila: " + tila);
                                                   System.out.println("");
                                               }
                                           }
                                       ...
                                       if (apitamoOut.isLiitteitaVastaanotettu()) {
                                           System.out.println("Yksi tai useampi liitetiedosto on siirretty Verohallinnon k‰sittelyyn.");
                                       ...
                                       System.out.println("L‰hetystapahtuma ep‰onnistui.");
                           ...
                           if (katsoOut.onTunnistusOk()) {
                               ...
                               if (apitamoOut != null) {
                                   if (apitamoOut.isLahetysTapahtumaOk()) {
                                       ...
                                       System.out.println("Nouto ep‰onnistui.");
   			        	...
   			        	System.out.println();
   			        	System.out.println("	Login         - Ohjelma kysyy tunnistustiedot ja tunnistautuu WSIDP:lle");
   			        	System.out.println("	Laheta        - Lahetetaan ilmoitus- ja/tai liitetiedostoja ApiTaMolle");
   			        	System.out.println("	Hae           - Vastausaineiston haku (esim. veronumerot ja suorasiirrot)");
   			        	...
   			        	//tulostetaan kirjautuneen k‰ytt‰j‰n tiedot
   			        	System.out.println();
   			        	if (katsoOut.onTunnistusOk()) {
    ...
    private static StringBuffer lueLahdeTiedot(String tied){
   		...
   		try{
   			...
   			//tyhj‰t‰‰n sbuf varmuuden vuoksi
   			sbuf.setLength(0);
    ...
    private static KatsoOutDto tunnistaudu(KatsoInDto in){
    	...
    	try{
	        ...
	        if (loginType == null || loginType.length() == 0 || (!loginType.equals("1") &&
	        		!loginType.equals("2"))){
	        	return null;
	        ...
	        //kun kaikki on kyselty, kirjaudutaan sis‰‰n
    ...
    private static int keraaLiitteet(Vector<String> liitteet){
    	...
    	try{
			...
			//tarkastetaan ett‰ liitteet ovat oikeita tiedostoja
			for(int i = 0; i < liitteet.size(); i++){
				...
				if (file.exists() && file.canRead()){
					...
					System.out.println();
					System.out.println("Liitetiedostoa '" + liitteet.elementAt(i) + "' ei loydy tai lukeminen on estetty, liitetta ei laheteta.");
    ...
    private static ApitamoOutDto laheta(KatsoOutDto katsoOut){
    	...
    	try {
            ...
            if (onFile){
                ...
                //tutkitaan ett‰ annettu tiedosto on olemassa ja sit‰ voi lukea...
            ...
            //kysell‰‰n mahdolliset liitteet
            System.out.print("Lahetetaanko liitetiedostoja (k/e)?: ");
            stdin = new BufferedReader(new InputStreamReader(System.in));
            tmp = stdin.readLine();
            tmp = tmp.trim();

            //tyhjennet‰‰n n‰m‰ varmuuden vuoksi
            liiteLkm = 0;
            liitteet.removeAllElements();
            ...
            // kysell‰‰n email
            System.out.println("Jos olet lahettamassa palautettavaa aineistoa voit saada sahkopostiisi ilmoituksen aineiston valmistuttua.");
            System.out.println("Jos olet lahettamassa Rakentamisen tiedonantomenettelyyn liittyvaa perusilmoitusta voit saada sahkopostiisi aineiston ilmoitustunnisteen.");
            System.out.println("Mik‰li et halua ilmoitusta sahkopostiisi tai olet lahettamassa muunlaisia ilmoituksia, paina suoraan <enter>");
            System.out.println("");
